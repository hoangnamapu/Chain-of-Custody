...
def handle_checkout(args):
    """Handles the 'bchoc checkout' command to check out an evidence item."""
    ...
    if not blockchain_file_path:
        print("CRITICAL: BCHOC_FILE_PATH environment variable not set. Cannot checkout item.", file=sys.stderr)
        sys.exit(1)
    ...
    # Check if the item is already in CHECKEDOUT state
